...
router.get('/housingloan', async (req, res) => {
    const { amount, maturity } = req.query;
    if (!amount || !maturity) return res.status(400).json({ success: false, message: 'Invalid queries' });
    let houseLoans = await houseLoansModel.find({ 'creditTerm.min': { $lte: maturity }, 'creditTerm.max': { $gte: maturity }, 'creditAmount.min': { $lte: amount }, 'creditAmount.max': { $gte: amount } }).select('-_id -creditAmount -creditTerm').populate({ path: 'bank', select: '-_id -houseLoans' });
...
router.get('/housingloan/detail', async (req, res) => {
